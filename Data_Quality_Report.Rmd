---
title: "Data Quality Control Report for `r params$file_name`"
date: "`r params$report_date`"
output: html_document
params:
  file_path: "test_that/404_Do_2008.xlsx"
  file_name: "404_Do_2008"
  report_date: !r format(Sys.Date(), "%Y-%m-%d")
  researcher: "Unknown"
---
...
htmltools::HTML(sprintf(
  '<div style="margin-bottom: 20px;">
     ...
     <p><strong>Unique IDref:</strong> <span style="color:%s;">%d</span></p>
   </div>',
  params$researcher,
  contributor_text,
  if (n_IDref > 1) "red" else "black",
  n_IDref
))
...
if (nrow(invalid_levels) > 0) {
  htmltools::HTML('<h3 style="background-color:#f8d7da; padding:8px 12px; border-radius:6px; font-size:1.17em; font-weight:bold; margin-top:1em; color:#721c24;">
  ❌ Invalid values in \'Level\' column
  </h3>') %>% print()
...
htmltools::HTML(sprintf(
  '<h3 style="background-color:%s; padding:8px 12px; border-radius:6px; font-size:1.17em; font-weight:bold; margin-top:1em;">%s</h3>',
  bg_color, message_text
))
...
if (exists("transformation") && ncol(transformation) > 0) {
...
htmltools::HTML("<h3>📋 Observations</h3>")
htmltools::HTML("<h5>Shows a list of all distinct non-empty comments found in the dataset.</h5>")

if ("Observations" %in% names(df)) {
  unique_observations <- na.omit(unique(trimws(df$Observations)))

  if (length(unique_observations) > 0) {
    obs_table <- tibble(`Observation Text` = unique_observations)
    formattable(obs_table)
  } else {
    htmltools::HTML("<p><i>No contributor comments found.</i></p>")
  }
} else {
  htmltools::HTML("<p><i>Column 'Observations' not found in dataset.</i></p>")
}
